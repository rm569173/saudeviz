...
# MAGIC
# MAGIC Primeira camada do Medallion. Registra as três fontes do desafio como
# MAGIC tabelas Delta, **sem nenhuma transformação de conteúdo**: o que entra é
# MAGIC exatamente o que o DATASUS, a API do CNES e o IBGE publicaram.
# MAGIC
# MAGIC | Fonte | Formato original | Origem |
# MAGIC |---|---|---|
# MAGIC | 1 — SIH/SUS | `.dbc` (DBF comprimido) | FTP público do DATASUS |
# MAGIC | 2 — CNES estabelecimentos | JSON | API REST do Ministério da Saúde |
# MAGIC | 2b — CNES leitos | `.dbc` | FTP público do DATASUS |
# MAGIC | 3 — População municipal | CSV | API do IBGE |
# MAGIC
# MAGIC ### Por que a decodificação do `.dbc` roda fora do Databricks
# MAGIC
# MAGIC O `.dbc` é um DBF comprimido com PKWare DCL, formato proprietário do
# MAGIC DATASUS. A decodificação exige a extensão C `pyreaddbc` e acesso FTP de
# MAGIC saída — duas coisas que não se resolvem bem em compute serverless.
# MAGIC
# MAGIC Isso segue o padrão de lakehouse: a ferramenta de ingestão *pousa* o
# MAGIC arquivo na landing zone, e o lakehouse começa no Bronze. O código do
# MAGIC ingestor está versionado em `src/ingestao/` e é reproduzível com
# MAGIC `py -m src.ingestao.extrai_sih`.
...
# MAGIC
# MAGIC Os parquets estão particionados por UF no diretório. Lemos com
# MAGIC `basePath` para que o Spark reconheça a estrutura, e derivamos a UF do
# MAGIC próprio caminho do arquivo — assim a partição não depende de nenhuma
# MAGIC coluna interna do DBF.
...
# MAGIC
# MAGIC O JSON da API tem atributos que variam entre estabelecimentos — é o
# MAGIC dado semiestruturado exigido pelo desafio. Ele chega já achatado em
# MAGIC parquet pelo ingestor, mas o JSON original fica preservado em
# MAGIC `dados/raw/cnes/estabelecimentos.jsonl` no repositório.
...
# MAGIC
# MAGIC Lemos o **CSV original**, e não o parquet: o desafio pede explicitamente
# MAGIC o uso do formato CSV como terceira fonte. No Oracle este mesmo arquivo
# MAGIC seria uma `EXTERNAL TABLE` — ver `src/db/ddl_oracle.sql`.
...
# MAGIC ### Próximo passo
# MAGIC
# MAGIC O notebook `02_prata` aplica limpeza, tipagem, decodificação dos domínios
# MAGIC do SIH e — o mais importante — troca a dimensão temporal da competência
# MAGIC de pagamento para a **data real de internação**.
