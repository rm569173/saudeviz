...
--------------------------
O Select AI da Oracle so existe no Autonomous Database. A instancia academica
da FIAP e um Oracle 19c Enterprise, sem o pacote DBMS_CLOUD_AI (verificado por
consulta a all_objects). Implementamos entao o mecanismo equivalente sobre os
MESMOS metadados que o Select AI consumiria: os COMMENT ON das tabelas
T_SAUDE_*, catalogados em src/selectai/metadados.py.

O que ele e, e o que ele nao e
------------------------------
E um tradutor DETERMINISTICO: classifica a intencao da pergunta por vocabulario,
extrai entidades (UF, periodo, quantidade, metrica) e monta o SQL a partir de
modelos parametrizados. O SQL gerado e real, executado no banco, e exibido ao
usuario - exatamente o comportamento de "SELECT AI showsql".

NAO e um modelo de linguagem. Nao generaliza para perguntas fora do repertorio
mapeado, e diz isso claramente em vez de inventar uma resposta. Num painel de
saude publica, um tradutor que admite nao ter entendido e preferivel a um que
devolve um numero errado com confianca.
"""
...
# Vocabulario de cada intencao.
#
# Sao RADICAIS, nao palavras inteiras: "transfer" reconhece transferencia,
# transferem e transferido. Cada termo tem um peso - expressoes especificas
# valem mais que termos genericos, para que "quais hospitais mais transferem"
# caia em transferencias e nao no ranking geral de hospitais.
#
# O casamento usa fronteira de palavra a esquerda: sem isso "hospital"
# casaria dentro de "hospitalar" e a intencao generica roubaria perguntas
# especificas sobre capacidade.
INTENCOES = {
    "municipios_criticos": {
